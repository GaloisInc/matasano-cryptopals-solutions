...
import Set1
import Set2.Helpers
import Set3.C19 ( keystream, scoreString )


{-

the approach taken here is like the repeating-XOR of C3. we truncate the ciphertexts to
the length of the shortest, then transpose them, so that the first list contains the first
byte of each ciphertext, the second list contains the seconds, and so on. we guess a key and
use an english character frequency table to score guesses. we pick the best and return the
guessed key

-}

guessKeystreamFromCiphertexts :: [[Byte]] -> [Byte]
guessKeystreamFromCiphertexts cts = map chooseKeyByte transposedCTs
  where
    minLen = minimum (map length cts)
    transposedCTs = transpose (map (take minLen) cts)

chooseKeyByte :: [Byte] -> Byte
chooseKeyByte [] = error "chooseKeyByte: empty input"
chooseKeyByte col = fst $ head $ sortBy (comparing snd) $
  [ (k, scoreWithCapitals (asciify (col `xor'` (repeat k))))
    | k <- [0..255]
  ]

scoreWithCapitals :: String -> Double
scoreWithCapitals str = sum (map f str)
                      / fromIntegral (length str)
  where
  f :: Char -> Double
  f c = case lookup c englishDistr of
          -- penalize characters not in our frequency table
          -- (letters and spaces)
          Nothing -> 17
          Just sc -> -sc


c20Ciphertexts seed = map ((keystream seed `xor'`) . decodeBase64)
